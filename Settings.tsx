...
const Settings = () => {
    ...
    const loadUserData = async () => {
      ...
      if (profile.user_type === 'founder') {
        ...
        if (founderProfile) {
          ...
          setFounderVideoUrl(founderProfile.video_url || "");
  ...
  const handleVideoUpload = async (e: React.ChangeEvent<HTMLInputElement>) => {
    const file = e.target.files?.[0];
    if (!file || !userId || userType !== 'founder') return;
            ...
            <CardContent className="space-y-4">
              ...
              <div className="space-y-4 pt-4 border-t">
                <div className="space-y-2">
                  ...
                  <div 
                    className="relative h-48 rounded-lg bg-gradient-to-br from-primary/20 to-accent/20 overflow-hidden cursor-pointer group"
                    onClick={() => videoInputRef.current?.click()}
                  >
                    {founderVideoUrl ? (
                      <video src={founderVideoUrl} className="w-full h-full object-cover" controls={false} />
                    ) : (
                      <div className="w-full h-full flex items-center justify-center">
