...
BEGIN
  ...
  IF v_user_type = 'founder' THEN
    ...
    IF jsonb_typeof(meta->'traction_tiles') = 'array' THEN
      SELECT array_agg(value::text) INTO v_tiles
      FROM jsonb_array_elements_text(meta->'traction_tiles');
    END IF;
    ...
    INSERT INTO public.founder_profiles (
      ...
      growth_mom, paying_customers, operations_start_date, team_full_time,
      waitlist_signups, active_users, pilots_lois, product_status,
      user_growth_mom, traction_tiles
    )
    VALUES (
      NEW.id,
      ...
      NULLIF(meta->>'growth_mom', ''),
      NULLIF(meta->>'paying_customers', '')::int,
      NULLIF(meta->>'operations_start_date', '')::date,
      COALESCE((meta->>'team_full_time')::boolean, false),
      NULLIF(meta->>'waitlist_signups', '')::int,
      NULLIF(meta->>'active_users', ''),
      NULLIF(meta->>'pilots_lois', '')::int,
      NULLIF(meta->>'product_status', ''),
      NULLIF(meta->>'user_growth_mom', ''),
      COALESCE(v_tiles, '{}'::text[])
    )
